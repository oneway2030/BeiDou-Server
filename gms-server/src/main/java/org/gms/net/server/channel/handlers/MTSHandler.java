                ...
                case 2: { //put item up for sale
                    ...
                    if (i != null && c.getPlayer().getMeso() >= 5000) {
                        try (Connection con = DatabaseConnection.getConnection();
                            ...
                            if (rs.next()) {
                                if (rs.getInt(1) > 50) { // They have more than 10 items up for sale already!
                                    c.getPlayer().dropMessage(1, "最多只能上架50件商品!");
                ...
                case 17://buy from cart
                    ...
                    try (Connection con = DatabaseConnection.getConnection();
                        ...
                        if (rs.next()) {
                            ...
                            int price = orgPrice + 100 + (int) (orgPrice * 0.1); // taxes
                            ...
                            if (c.getPlayer().getCashShop().getCash(CashShop.NX_CREDIT) >= price) { // FIX
                                ...
                                for (World world : worlds) {
                                    ...
                                    if (victim != null) {
                                        victim.getCashShop().gainCash(1, orgPrice);
                                        alwaysnull = false;
                                        break;
                                    }
                                }
...
//                                        pse.setInt(1, orgPrice); // 要增加的meso数量（原有的orgPrice，即卖家应得金额）
